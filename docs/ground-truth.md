...
| G10 | MWDAT version pinned? | **0.9.0, `exactVersion`**, from `github.com/facebook/meta-wearables-dat-ios`. Adopted for `startUnregistration()` and the typed registration errors. DAM is not a choice any more — 0.9.0 always enables it and ignores the `MWDAT.DAMEnabled` key, which this app never set. **0.9.0 raised the SDK's own minimum to iOS 17.2** (`minos 17.2` in the shipped binary), so the app target's deployment target is 17.2; it was 15.1, which would have installed on devices that cannot load the framework. | `ios/Clypso.xcodeproj/project.pbxproj:757–761`; `IPHONEOS_DEPLOYMENT_TARGET` at `:497`, `:526`, `:617`, `:693` |
