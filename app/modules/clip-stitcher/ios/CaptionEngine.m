...
/// Reads the whole audio track as 16 kHz mono 16-bit PCM and normalizes its
/// level. Glasses audio is quiet and far-field; recognition on the raw track
/// misses words that a normalized copy hears. The clip's own audio is never
/// touched - this is a throwaway copy for the recognizer.
static NSData *CEReadNormalizedPCM(AVAsset *asset)
{
  ...
  for (NSUInteger i = 0; i < count; i++) {
    int32_t magnitude = abs(samples[i]);
    if (magnitude > peak) {
      peak = magnitude;
    }
  }
  // Leave headroom rather than pushing to full scale - clipping costs more
  // recognition accuracy than the extra loudness buys.
  double gain = (double)(INT16_MAX * 0.89) / (double)peak;
  if (gain > 1.0) {
    for (NSUInteger i = 0; i < count; i++) {
